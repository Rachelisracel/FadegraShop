    ...
    <header class="bg-forest sticky top-0 z-50 border-b border-white/10">
        <div class="max-w-7xl mx-auto px-6 py-5 flex items-center justify-between">
            ...
            <div class="flex items-center space-x-6 text-white text-lg">
                @if(Auth::check())
                    @php $userRole = Auth::user()->roleRelation->name ?? ''; @endphp
                    @if(in_array($userRole, ['admin', 'staff']))
                        <a href="{{ route('admin.orders.index') }}" class="hover:text-cream-accent transition-colors text-sm" title="Quản lý đơn hàng">
                            <i class="fa-solid fa-receipt"></i>
                        </a>
                    @else
                        <a href="{{ route('orders.index') }}" class="hover:text-cream-accent transition-colors text-sm" title="Lịch sử đơn hàng">
                            <i class="fa-solid fa-receipt"></i>
                        </a>
                    @endif
