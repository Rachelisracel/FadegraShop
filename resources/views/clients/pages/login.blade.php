    ...
    <div class="min-h-[calc(100vh-76px)] flex bg-[#F8F6F2]">
        ...
        <div class="hidden lg:block lg:w-1/2 relative overflow-hidden">
            ...
            <div class="absolute bottom-20 left-16 text-white max-w-md z-10">
                ...
                <p class="text-base text-white/80 leading-relaxed">Cùng Fadegra thưởng thức những ly trà mộc mạc và tận
                    hưởng phút giây thư giãn trọn vẹn nhất hôm nay.</p>
            </div>
        </div>
        ...
        <div class="w-full lg:w-1/2 flex items-center justify-center p-6 sm:p-12">
            <div class="w-full max-w-md bg-white p-8 sm:p-10 rounded-3xl shadow-sm border border-black/5">
                ...
                <!-- NẾU CÓ THÔNG BÁO TỪ TRANG ĐĂNG KÝ CHUYỂN QUA THÌ HIỂN THỊ -->
                @if(session('success'))
                    <div
                ...
                <form action="{{ route('login.post') }}" method="POST" class="space-y-5">
                    ...
                    <div>
                        <div class="flex justify-between items-center mb-1.5">
                            ...
                            <a href="{{ url('/forgot-password') }}"
                                class="text-xs font-medium text-[#354A3D] hover:underline">Quên mật khẩu?</a>
                        </div>
                    ...
                    <div class="flex items-center gap-2 pt-1">
                        <input type="checkbox" id="remember"
                            class="w-4 h-4 text-[#354A3D] rounded border-gray-300 focus:ring-[#354A3D] accent-[#354A3D]">
                        <label for="remember" class="text-sm text-gray-600 cursor-pointer">Ghi nhớ đăng nhập</label>
                    </div>

                    <!-- Nút Submit -->
                    <button type="submit"
                        class="w-full bg-[#354A3D] text-white font-bold rounded-xl py-3.5 mt-4 shadow-md hover:bg-[#2A4435] transition-colors flex justify-center items-center gap-2">
                        Đăng nhập
                ...
                <div class="mt-3 text-center text-sm text-gray-500">
                    <a href="{{ route('orders.lookup.form') }}" class="font-medium text-[#354A3D] hover:underline flex items-center justify-center gap-1.5">
                        <i class="fa-solid fa-magnifying-glass text-xs"></i> Tra cứu đơn hàng không cần đăng nhập
                    </a>
                </div>
